...
% CVSId = '$Id: showsrs2.m,v 1.112 2011/02/24 17:09:46 gadde Exp $';
% CVSRevision = '$Revision: 1.112 $';
% CVSDate = '$Date: 2011/02/24 17:09:46 $';
  ...
  elseif ~ischar(varargin{1})  % LAUNCH GUI
    ...
    for c=1:length(otherInfo.imageNames)
      ...
      if ~isempty(otherInfo.imageNames{c})
        ...
        [pathstr,name,ext] = fileparts(otherInfo.imageNames{c});
        currLabel = [name,ext];
        if ~isempty(pathstr)
    ...
    set(handles.imgAx,'UserData',struct(...
      'getImageString','squeeze(permute(image_ud.srs(:,:,z,timePts(n),:),[2 1 3 4 5]))', ...
...
if isstruct(varargin{1}) & isfield(varargin{1},'data')
  ...
  if isfield(varargin{1},'info') & ...
        strcmp(varargin{1}.info.elemtype, 'rgb24')
    % we have RGB data
    baseisrgb = 1;
    sizes = size(varargin{1}.data);
    if sizes(end) ~= 3
      emsg = sprintf(['Base image is labeled as rgb24 but last dimension does not have a size of 3!']);
      error(emsg);
    end
    % force R/G/B dimension as 5th dimension (and add singleton
    % dimensions before, if necessary) to avoid any confusion
    if length(sizes) < 5
      baseInfo.srs = ...
	  reshape(varargin{1}.data, [sizes(1:end-1) repmat([1], [1 (5 - length(sizes))]) sizes(end)]);
    else
...
% Check base image
if ~(any(ndims(baseInfo.srs) == [2 3 4]) | (baseisrgb & ndims(baseInfo.srs) == 5)) | ~(isnumeric(baseInfo.srs) | islogical(baseInfo.srs)) | issparse(baseInfo.srs)
  emsg='base image must be a 2-, 3-, or 4-D non-sparse numeric or logical array'; error(emsg);
end
...
if isfield(baseInfo,'info') & size(baseInfo.srs,3) > 1
  ...
  if ~isempty(infoVoxelSize) & all(infoVoxelSize == 0)
    waitfor(warndlg(sprintf(['Voxel spacing for all dimensions is zero.\n',...
		             'Setting it to an arbitrary value (1).\n']), ...
		    'Unable to determine voxel size','modal'));
    infoVoxelSize = infoVoxelSize + 1;
  end
...
if strcmp(get(handles.UseTransMenu,'Checked'),'on')
  ...
  for n = layers2display
    ...
    for allImages_h_n = 1:length(allImages_h)
      ...
      if ndims(currImageData) == 2
	% if currImageData is not RGB, then add the 3rd (singleton) dimension
	currImageData = reshape(currImageData, [size(currImageData) 1]);
      end
      if fig_ud.delayInterp
        ...
        if any(image_ud.scale~=1)
          ...
          if ~((ax_ud.toScreenPlane(3) == 1 & (x > size(image_ud.srs,1))) | ...
            ...
            currImageData2 = eval(ax_ud.getImageString);  % Get the next raw image data
	    if ndims(currImageData2) == 2
	      % if currImageData is not RGB, then add the 3rd (singleton) dimension
	      currImageData2 = reshape(currImageData2, [size(currImageData2) 1]);
	    end
	  else
	    currImageData2 = currImageData;
	  end
	  for rgbind = 1:size(currImageData,3)
            % Interpolate using trilinear on adjacent slices
	    currRGBslice1 = currImageData(:,:,rgbind);
	    currRGBslice2 = currImageData2(:,:,rgbind);
            tempImData = zeros([size(currRGBslice),2]);
            tempImData(:,:,1) = currRGBslice1;
            tempImData(:,:,2) = currRGBslice2;
            ...
            currImageData(:,:,rgbind) = ...
		tempImData(:,:,mod(loc(ax_ud.toScreenPlane(3))-1,...
      ...
      if ~isempty(maskString)
        mask = local_calcmask(handles,allImages_h(allImages_h_n),...
          loc,maskString);
        currImageData = currImageData .* repmat(mask, [1 1 size(currImageData,3)]);
      end

      % Find the activation clusters of overlays
      clMask = [];
      if ~isempty(clSize)
        clMask = zeros(size(currImageData,1)*size(currImageData,2));
        clMask(find(max(currImageData,3) >= cmapLim(1))) = 1;
        clMask = bwareaopen(clMask,clSize);
        currImageData = currImageData .* repmat(clMask, [1 1 size(currImageData,3)]);
      end

      % Display image (convert to RGB & calculate transLim mask)
      rgbdata = [];
      if size(currImageData, 3) == 3
	rgbdata = uint8(max(1,min(256,fix(((currImageData - cmapLim(1)) ./ diff(cmapLim)) .* 256) + 1)));
      else
	rgbdata = scaled2rgb(currImageData,image_ud.cmap,cmapLim);
      end
      if n == 1 % Base
        % Just display CData for base image
        set(allImages_h(allImages_h_n),'CData',rgbdata);
      else % Overlays
  ...
  for allImages_h_n = 1:length(allImages_h)
	...
	currImageData = reshape(currImageData, [size(currImageData) 1]);
      ...
      if fig_ud.delayInterp
	    ...
	    if ndims(currImageData2) == 2
	      % if currImageData is not RGB, then add the 3rd (singleton) dimension
	      currImageData2 = reshape(currImageData2, [size(currImageData) 1]);
	    end
	  else
	    currImageData2 = currImageData;
	  end
      ...
      currImageDataRGB = [];
      if size(currImageData, 3) == 3
	currImageDataRGB = uint8(max(1,min(256,fix(((currImageData - cmapLim(n,1)) ./ diff(cmapLim(n,:))) .* 256) + 1)));
      else
...
if any(strcmp(currOrientMenuTag,{'AxialOrientMenu','CoronalOrientMenu','SagittalOrientMenu'}))
  ...
  switch currOrientMenuTag
    case 'AxialOrientMenu'
      ...
      ax_ud.getImageString='squeeze(permute(image_ud.srs(:,:,z,timePts(n),:),[2 1 3 4 5]))';
    ...
    case 'CoronalOrientMenu'
      ...
      ax_ud.getImageString='squeeze(permute(image_ud.srs(:,y,:,timePts(n),:),[3 1 2 4 5]))';
    ...
    case 'SagittalOrientMenu'
      ...
      ax_ud.getImageString='squeeze(permute(image_ud.srs(x,:,:,timePts(n),:),[3 2 1 4 5]))';
...
elseif strcmp(currOrientMenuTag,'ThreePlaneOrientMenu')
  ...
  set(ax_h(2),'DataAspectRatio',voxelDataAspect([2 1 3]), ...
    'XLim',[0.5,currImageSize3D(1)+0.5],'YLim',[0.5,currImageSize3D(2)+0.5], ...
    'UserData',struct('getImageString','squeeze(permute(image_ud.srs(:,:,z,timePts(n),:),[2 1 3 4 5]))', ...
  ...
  set(handles.imgAx,'DataAspectRatio',voxelDataAspect([2 1 3]), ...
    'XLim',[0.5,currImageSize3D(1)+0.5],'YLim',[0.5,currImageSize3D(2)+0.5], ...
    'UserData',struct('getImageString','squeeze(permute(image_ud.srs(:,y,:,timePts(n),:),[3 1 2 4 5]))', ...
  ...
  set(ax_h(1),'DataAspectRatio',voxelDataAspect([2 1 3]), ...
    'XLim',[0.5,currImageSize3D(1)+0.5],'YLim',[0.5,currImageSize3D(2)+0.5], ...
    'UserData',struct('getImageString','squeeze(permute(image_ud.srs(x,:,:,timePts(n),:),[3 2 1 4 5]))', ...
...
%
% $Log: showsrs2.m,v $
% Revision 1.112  2011/02/24 17:09:46  gadde
% Add RGB support to showsrs2
%
