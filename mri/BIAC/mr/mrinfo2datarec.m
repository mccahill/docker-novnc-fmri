...
% CVSId = '$Id: mrinfo2datarec.m,v 1.27 2007/02/23 19:51:26 gadde Exp $';
% CVSRevision = '$Revision: 1.27 $';
% CVSDate = '$Date: 2007/02/23 19:51:26 $';
...
try
  ...
  for dimnum=1:length(info.dimensions)
    ...
    dim = [];
    childrank = childrank + 1;
    dim.CHILDRANK = childrank;
  ...
  if isfield(info, 'frags') & isfield(info.frags, 'filenames')
    for fragnum=1:length(info.frags.filenames)
      filename = info.frags.filenames{fragnum};
      fileoffset = info.frags.fileoffsets(fragnum);
      fragsize = info.frags.fragsizes(fragnum);
      datarec.filename{fragnum}.VALUE = filename;
      childrank = childrank + 1;
      datarec.filename{fragnum}.CHILDRANK = childrank;
      if isfinite(fileoffset) & fileoffset >= 0
        datarec.fileoffset{fragnum}.VALUE = fileoffset;
        childrank = childrank + 1;
        datarec.fileoffset{fragnum}.CHILDRANK = childrank;
      end
      if isfinite(fragsize) & fragsize >= 0
        datarec.filerecordsize{fragnum}.VALUE = fragsize;
        childrank = childrank + 1;
        datarec.filerecordsize{fragnum}.CHILDRANK = childrank;
      end
    end
  end
...
% Revision 1.27  2007/02/23 19:51:26  gadde
% Explicitly order the element children of datarec.
%
% Revision 1.26  2006/06/29 20:16:48  gadde
% Fix a missing semicolon.
%
% Revision 1.25  2006/06/28 18:20:37  gadde
