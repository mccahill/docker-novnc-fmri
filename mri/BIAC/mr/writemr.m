...
% CVSId = '$Id: writemr.m,v 1.35 2011/02/15 15:47:36 petty Exp $';
CVSRevision = '$Revision: 1.35 $';
% CVSDate = '$Date: 2011/02/15 15:47:36 $';
...
elseif strcmp('Float', outputformat) | ...
  ...
  for vnum = 1:sizes(4)
    ...
    volfilename = fullfile(pathstr,[name ext]);
...
elseif strcmp('BXH', outputformat)
  ...
  if length(outputopts) >= 3
    ...
    fulldatafilename = fullfile(pathstr, [name '.data']);
  end
  [datapathstr, dataname, dataext] = fileparts(fulldatafilename);
...
if ~nobxh | strcmp('BXH', outputformat)
  ...
  [bxhpathstr, bxhname, bxhext] = fileparts(fullbxhfilename);
  ...
  fullbxhfilename = fullfile(bxhpathstr, [bxhname bxhext]);
  ...
  for i = 1:length(frags.filenames)
    ...
    [fragpathstr, fragname, fragext] = fileparts(frags.filenames{i});
    if strcmp(fragpathstr, bxhpathstr)
      frags.filenames{i} = [fragname fragext];
    end
  end
...
% $Log: writemr.m,v $
% Revision 1.35  2011/02/15 15:47:36  petty
% *** empty log message ***
%
