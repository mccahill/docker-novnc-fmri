...
% CVSId = '$Id: showsrs2_cfg.m,v 1.27 2009/04/23 18:58:17 bizzell Exp $';
% CVSRevision = '$Revision: 1.27 $';
% CVSDate = '$Date: 2009/04/23 18:58:17 $';
...
elseif ~ischar(varargin{1})  % LAUNCH GUI
  ...
  if ~isempty(gui_h)
    ...
    %     handles.showsrs2_WinlevButton = handle of the W+L button where the "fast
    %     layer" layerNum, cmapLim, transLim, clusterSize and maskString are stored.
    ...
    if length(showsrs2UD.image_names) > 1
      set(handles.controlMaskMenu,'String',{'No Mask',showsrs2UD.image_names{2:end},'Advanced Mask'});
    end
...
% --------------------------------------------------------------------
function enableClustering_Callback(h, eventdata, handles)
% Enable/disable clustering edit box

if get(handles.enableClustering,'Value')
  % Enable editing of cluster size edit box
  set(handles.clusterSizeEdit,'String','','Enable','on');
  set(handles.clusterSizeText,'Enable','on');
else
  % Disable editing of cluster size edit box
  set(handles.clusterSizeEdit,'String','','Enable','off');
  set(handles.clusterSizeText,'Enable','off');
  % Update the clustering on the image
  local_applyClustering(handles,[]);
end

% --------------------------------------------------------------------
function setCluster_Callback(h, eventdata, handles)
% This function sets the minimum cluster size for display

% Get setting from edit boxes
clSize = str2num(get(handles.clusterSizeEdit,'String'));
if isempty(clSize) | clSize < 0 % Catch invalid strings
  waitfor(warndlg('Invalid cluster size!', 'Invalid cluster size','modal'));
  clSize = [];
  set(handles.clusterSizeEdit,'String','');
end

% Update the clustering on the image
local_applyClustering(handles,clSize);

% --------------------------------------------------------------------
function controlMaskMenu_Callback(h, eventdata, handles)
% This function will set the mask string

cmapLim = [0,0];
cmapLim(1) = str2num(get(handles.lowerClimEdit,'String'));
cmapLim(2) = str2num(get(handles.upperClimEdit,'String'));
transLim = str2num(get(handles.transLimitEdit,'String'));

% Set transLim to -Inf for base image since it is not relevant.
if handles.isBaseImage, transLim = -Inf;  clusterSize = []; maskString = '';
else
  % Return current cluster size
  clusterSize = str2num(get(handles.clusterSizeEdit,'String'));
  maskString = get(handles.controlMaskMenu,'String');
  maskStrIdx = get(handles.controlMaskMenu,'Value');
  maskString = maskString{maskStrIdx};
end

% If no mask selected, set maskString empty
if strcmp(maskString,'No Mask'), maskString = ''; end

% Advanced mask has not been implemented
if strcmp(maskString,'Advanced Mask')
  warning('Advanced mask not implemented.');
  maskString = '';
  set(handles.controlMaskMenu,'Value',1);
end

% Update FastInfo array {layerNum,cmapLim,transLim,clusterSize,maskString}
set(handles.showsrs2_WinlevButton,'UserData',...
  {handles.currImageIndex,cmapLim,transLim,clusterSize,maskString});
...
if ~handles.isBaseImage % Don't do this for base images
  ...
  % Enable/disable clustering edit box, checkbox and label
  if isempty(imageUD.clusterSize)
    % Cluster size not set
    set(handles.enableClustering,'Value',0,'Enable','on');
    set(handles.clusterSizeEdit,'String','','Enable','off');
    set(handles.clusterSizeText,'Enable','off');
  else
    % Cluster size set
    set(handles.enableClustering,'Value',1,'Enable','on');
    set(handles.clusterSizeEdit,'String',num2str(imageUD.clusterSize),'Enable','on');
    set(handles.clusterSizeText,'Enable','on');
  end

  % Enable the masking menu
  set(handles.controlMaskMenu,'Enable','on');
  if isempty(imageUD.maskString)
    set(handles.controlMaskMenu,'Value',1);
  else
    set(handles.controlMaskMenu,'Value',strmatch(...
      imageUD.maskString,get(handles.controlMaskMenu,'String')));
  end

else % Base image doesn't have transparency, clustering or masking
  ...
  % Disable the Clustering related items in the GUI
  set(handles.clusterSizeEdit,'String','','Enable','off');
  set(handles.clusterSizeText,'Enable','off');
  set(handles.enableClustering,'Value',0,'Enable','off');

  % Disable the masking menu
  set(handles.controlMaskMenu,'Enable','off');
end
...
%
%  Updates cmapLim, transLim & clusterSize fields in current image
%  UserData to reflect the setting in the showsrs_cfg GUI.
%
%  local_updateImageUserData(handles)
%    handles - structure of handles for showsrs2_cfg GUI
%
...
if ~handles.isBaseImage
  ...
  % If no mask selected, set maskString empty
  if strcmp(maskString,'No Mask'), maskString = ''; end
  imageUD.maskString = maskString;
  if get(handles.enableClustering,'Value')
    % Update the cluster size
    imageUD.clusterSize = str2num(get(handles.clusterSizeEdit,'String'));
  else
    imageUD.clusterSize = [];
  end
end
...
try
  ...
  % Update FastInfo array {layerNum,cmapLim,transLim,clSize,maskString
  % -> Turn off layerNum, clSize and maskString, and set cmapLim and
  %    transLim to dummy values (for no particular reason)
...
% -------------------- local_applyClustering --------------------------
function local_applyClustering(handles,clusterSize)
% local_applyClustering - Update the display for current clustering values
%
%  local_applyClustering(handles);
%    handles - structure of handles for showsrs2_cfg GUI
%    clusterSize - integer value of cluster size in number of voxels
...
if handles.isBaseImage, clusterSize = []; maskString = ''; end
...
% Set pos(2) because figure was going off screen
pos=get(fig,'Position'); pos(2) = pos(2)-(40-pos(4));
pos(3:4)=[40,43]; set(fig,'Position',pos);
...
handles.linkToCmapLimitCheckbox = uicontrol(fig,'Units','Characters','Style','checkbox', ...
  'Position',[12 24 25 1],'Tag','linkToCmapLimitCheckbox','String','Link to Colormap Limit', ...
  'Callback','showsrs2_cfg(''linkToCmapLimitCheckbox_Callback'',gcbo,[])', ...
  'Value',1);

% Clustering Label, Edit and Checkbox
handles.clusterSizeEdit = uicontrol(fig,'Units','Characters','Style','edit', ...
  'Position',[12 18 23 1.5],'Tag','clusterSizeEdit','String','', ...
  'Callback','showsrs2_cfg(''setCluster_Callback'',gcbo,[])');
handles.clusterSizeText = uicontrol(fig,'Units','Characters','Style','text', ...
  'Position',[12 19.5 23 1],'Tag','clusterSizeText','String','Min Cluster - # voxels');
handles.enableClustering = uicontrol(fig,'Units','Characters','Style','checkbox', ...
  'Position',[12 17 25 1],'Tag','enableClustering','String','Enable 2D Clustering', ...
  'Callback','showsrs2_cfg(''enableClustering_Callback'',gcbo,[])', ...
  'Value',0);

% Masking menu
handles.maskMenuText = uicontrol(fig,'Units','Characters','Style','text', ...
  'Position',[12 15.5 23 1],'Tag','maskMenuText','String','Set Mask');
handles.controlMaskMenu = uicontrol(fig,'Units','Characters','Style','popupmenu', ...
  'Position',[12 14 23 1.5],'Tag','controlMaskMenu','String','No Mask', ...
  'Callback','showsrs2_cfg(''controlMaskMenu_Callback'',gcbo,[])');
...
handles.colorbarAxes = axes('Units','Characters', ...
  'Position',[5 10 5 20.5],'Tag','colorbarAxes', ...
...
% Revision 1.27  2009/04/23 18:58:17  bizzell
% Added overlay by overlay masking options
%
% Revision 1.26  2006/06/21 18:05:15  gadde
% From Josh Bizzell: Implemented 2D clustering option for overlays
%
