...
% CVSId = '$Id: bxhabsorb.m,v 1.17 2011/02/15 15:47:34 petty Exp $';
...
if isempty(outputfile)
  ...
  [pathstr,name,ext] = fileparts(mrstruct.inputlocs{1});
  defaultName = fullfile(pathstr,[name,'.bxh']);
  ...
  while ~okay
    ...
    if isempty(ext), outputfile = fullfile(pathstr,[name,'.bxh']); end
    clear pathstr name ext
...
if isfield(mrstruct, 'inputfiles')
  for fnnum = 1:length(newdatarec.filename)
    [filePath,fileName,fileExt] = fileparts(newdatarec.filename{fnnum}.VALUE);
    if (caseSensitive & strcmp(filePath,pathstr)) ...
        | (~caseSensitive & strcmpi(filePath,pathstr))
      % Same directory, just keep the filename.
      newdatarec.filename{fnnum}.VALUE = [fileName,fileExt];
    end
  end
end
...
%
% $Log: bxhabsorb.m,v $
% Revision 1.17  2011/02/15 15:47:34  petty
% *** empty log message ***
%
