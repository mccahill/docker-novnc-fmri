...
% CVSId = '$Id: showsrs2_cfgtrans.m,v 1.9 2009/04/22 20:58:49 bizzell Exp $';
% CVSRevision = '$Revision: 1.9 $';
% CVSDate = '$Date: 2009/04/22 20:58:49 $';
...
showall_h=findobj(handles.showsrs2_cfgtransFig,'Tag','showAllCheckbox','Style','Checkbox');
if all(amap(2:end)==0), set(showall_h,'String','Show All'); set(showall_h,'Value',0); end
if all(amap(2:end)==1), set(showall_h,'String','Hide All'); set(showall_h,'Value',1); end
...
%Get the handle to the "ShowAll button"
showall_h=findobj(handles.showsrs2_cfgtransFig,'Tag','showAllCheckbox','Style','Checkbox');
...
if get(h,'Value')
  ...
  if get(showall_h,'Value'), set(showall_h,'Value',0); set(showall_h,'String','Show All'); end
end
...
if all(amap(2:end)>0), set(showall_h,'Value',1); set(showall_h,'String','Hide All'); end
...
if transLim == 0
  ...
  set(checkbox_h,'Value',0);
  local_setUserDataField(checkbox_h,'storedTransLim',1);
  % If "ShowAll" on, turn off and change string
  if get(showall_h,'Value'), set(showall_h,'Value',0); set(showall_h,'String','Show All'); end
else
  ...
  % If all overlays shown, turn on "ShowAll" checkbox and change string
  if all(amap(2:end)>0), set(showall_h,'Value',1); set(showall_h,'String','Hide All'); end
end
...
% --------------------------------------------------------------------
function showAll_Callback(h, eventdata, handles)
% Show or hide all overlays

% Set all checkboxes to 1 or 0
chkbox_h=findobj(handles.showsrs2_cfgtransFig,'Tag','transCheckbox','Style','Checkbox');
if get(h,'Value')
  set(chkbox_h,'Value',1);
  set(h,'String','Hide All');
else 
  set(chkbox_h,'Value',0); 
  set(h,'String','Show All');
end

% Update AlphaMap (respective overlay stored in slider UserData.overlayNum)
amap=get(handles.showsrs2_UseTransMenu,'UserData'); % Get current AlphaMap

for n=1:length(chkbox_h)
  % Slider for this checkbox in application data
  slider_h = local_getUserDataField(chkbox_h(n),'slider_h');

  % Store the translim as 1
  local_setUserDataField(chkbox_h(n),'storedTransLim',1);
  set(slider_h,'Value',get(chkbox_h(n),'Value'));  % Set transslider
  % Update the AlphaMap
  amap(local_getUserDataField(slider_h,'overlayNum')+1)=get(slider_h,'Value');

end

% Set the AlphaMap in the handle
set(handles.showsrs2_UseTransMenu,'UserData',amap);
...
figSize = [45 blockHeight*nOverlays+figMargin(1)+figMargin(2)+...
...
pos = [figMargin(3)+allEqualButtonSize(1)+showAllMargin,figMargin(2),checkboxSize];
handles.ShowAll=uicontrol('Units','Characters','Tag','showAllCheckbox', ...
  'Style','Checkbox','Position',pos, ...
  'String','Show All','Min',0,'Max',1,...
  'Callback','showsrs2_cfgtrans(''showAll_Callback'',gcbo,[])');
...
% Revision 1.9  2009/04/22 20:58:49  bizzell
